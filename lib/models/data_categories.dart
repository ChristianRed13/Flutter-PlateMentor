import 'package:plate_mentor/models/category.dart';
import 'package:flutter/material.dart';

class DataCategories {
  // static final List<FoodCategory> categories = [
  //   FoodCategory(
  //     name: 'Keto',
  //     number: 30,
  //     color: Colors.blue.shade900,
  //     imageUrl: 'assets/keto.png',
  //   ),
  //   FoodCategory(
  //     name: 'Vegetarian',
  //     number: 300,
  //     color: Colors.lightGreen,
  //     imageUrl: 'assets/vegetarian.png',
  //   ),
  //   FoodCategory(
  //     name: 'High Protein',
  //     number: 57,
  //     color: Colors.purple,
  //     imageUrl: 'assets/high-protein.png',
  //   ),
  //   FoodCategory(
  //     name: 'Quick & Easy',
  //     number: 30,
  //     color: Colors.indigo,
  //     imageUrl: 'assets/quick&easy.png',
  //   ),
  //   FoodCategory(
  //     name: 'Seafood',
  //     number: 36,
  //     color: Colors.blueGrey.shade700,
  //     imageUrl: 'assets/seafood.png',
  //   ),
  //   FoodCategory(
  //     name: 'Breakfast',
  //     number: 36,
  //     color: Colors.yellow.shade700,
  //     imageUrl: 'assets/breakfast.png',
  //   ),
  //   FoodCategory(
  //     name: 'Dairy-Free',
  //     number: 36,
  //     color: Colors.blue,
  //     imageUrl: 'assets/dairy-free.png',
  //   ),
  //   FoodCategory(
  //     name: 'Gluten-Free',
  //     number: 36,
  //     color: Colors.deepOrange,
  //     imageUrl: 'assets/gluten-free.png',
  //   ),
  //   FoodCategory(
  //     name: 'Vegan',
  //     number: 36,
  //     color: Colors.lime.shade500,
  //     imageUrl: 'assets/vegan.png',
  //   ),
  //   FoodCategory(
  //     name: 'Sweet',
  //     number: 36,
  //     color: Colors.pink,
  //     imageUrl: 'assets/sweet.png',
  //   ),
  //   FoodCategory(
  //     name: 'Snack',
  //     number: 36,
  //     color: Colors.orange,
  //     imageUrl: 'assets/snack.png',
  //   ),
  //   FoodCategory(
  //     name: 'Low Carb',
  //     number: 36,
  //     color: Colors.red,
  //     imageUrl: 'assets/low-carb.png',
  //   ),
  //   FoodCategory(
  //     name: 'Budget',
  //     number: 36,
  //     color: Colors.lightGreenAccent.shade700,
  //     imageUrl: 'assets/budget.png',
  //   ),
  //   FoodCategory(
  //     name: 'Meat',
  //     number: 36,
  //     color: Colors.brown.shade600,
  //     imageUrl: 'assets/meat.png',
  //   ),
  //   FoodCategory(
  //     name: 'Low Fat',
  //     number: 36,
  //     color: Colors.lime.shade800,
  //     imageUrl: 'assets/budget.png',
  //   ),
  // ];
}
